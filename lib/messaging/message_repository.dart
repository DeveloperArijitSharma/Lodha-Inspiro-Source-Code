...
class MessageRepository {
  ...
  Future<InspiroMessage> sendMessage({
    ...
    if (userId == null) throw StateError('You must be signed in to send a message.');
  ...
  Future<InspiroMessage> editMessage({
    ...
    final row = await _client.from('messages').update({
      'text': text,
  ...
  Future<InspiroMessage> setReaction({
    ...
    final userId = _client.auth.currentUser?.id;
    if (userId == null) throw StateError('You must be signed in to react to a message.');

    final row = await _client.from('messages').select('reactions').eq('id', messageId).single();
    ...
    final updated = await _client.from('messages').update({'reactions': reactions}).eq('id', messageId).select().single();
