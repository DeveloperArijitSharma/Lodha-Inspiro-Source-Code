...
class _HomeScreenState extends State<HomeScreen> {
  ...
  Future<void> _fetchChatsFromSupabase() async {
    try {
      final response = await supabase.from('chats').select().order('created_at', ascending: false);
      setState(() {
        _chats.clear();
  ...
  Future<void> _deleteChat(String chatId, String chatName) async {
    ...
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        title: const Text('Delete Chat?', style: TextStyle(fontWeight: FontWeight.bold, fontFamily: 'Google Sans Flex')),
        content: Text('Delete conversation with $chatName?', style: const TextStyle(fontFamily: 'Google Sans Flex')),
        actions: [
          ...
          child: BackdropFilter(
            ...
            child: Container(
              ...
              decoration: BoxDecoration(
                color: isError ? Colors.redAccent.withOpacity(isDark ? 0.4 : 0.6) : _accentBlue.withOpacity(isDark ? 0.3 : 0.6),
                borderRadius: BorderRadius.circular(30),
                border: Border.all(color: Colors.white.withOpacity(0.3), width: 1.5),
              ),
              child: Text(message, textAlign: TextAlign.center, style: const TextStyle(color: Colors.white, fontWeight: FontWeight.bold, fontSize: 14, fontFamily: 'Google Sans Flex')),
            ),
          ),
        ),
      ),
  ...
  Future<void> _updateUserName(String newName) async {
    ...
    try {
      await supabase.auth.updateUser(UserAttributes(data: {'username': newName.trim()}));
          ...
          children: [
            Positioned.fill(
              child: AnimatedSwitcher(
                duration: smoothMotionNotifier.value ? const Duration(milliseconds: 180) : Duration.zero,
                child: _buildCurrentView(size, padding, isDark, key: ValueKey<int>(_currentIndex)),
              ),
            ),
            if (_currentIndex != 4)
              Positioned(left: size.width * 0.04, right: size.width * 0.04, bottom: size.height * 0.02, child: _buildLiquidGlassNavBar(size, isDark)),
          ],
        ),
      ),
    );
  ...
  Widget _buildCurrentView(Size size, double padding, bool isDark, {required Key key}) {
    switch (_currentIndex) {
      case 0: return _buildHomeScreenContent(size, padding, isDark);
      case 1: return _buildClassworkScreenContent(size, padding, isDark);
      case 2: return const NotebooksListScreen();
      case 3: return _buildChatTabContent(size, padding, isDark);
      case 4: return _buildSettingsScreenContent(size, padding, isDark);
      default: return _buildHomeScreenContent(size, padding, isDark);
    }
  }
  ...
  Widget _buildHeader(String title, Color textColor) {
    return Row(
      ...
      children: [
        Builder(builder: (context) => IconButton(padding: EdgeInsets.zero, constraints: const BoxConstraints(), icon: Icon(Icons.menu_rounded, color: textColor, size: 28), onPressed: () => Scaffold.of(context).openDrawer())),
        const SizedBox(width: 14),
        Text(title, style: TextStyle(color: textColor, fontSize: 28, fontWeight: FontWeight.bold, fontFamily: 'Google Sans Flex')),
      ],
    );
  }
  ...
  Widget _buildHomeScreenContent(Size size, double padding, bool isDark) {
    ...
    return SingleChildScrollView(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [_buildHeader('Live classes', textColor), SizedBox(height: size.height * 0.025), const HomeAiScreen()]),
    );
  }
  ...
  Widget _buildTaskCard(String title, String subtitle, Color badgeColor, bool isDark) {
    return Container(
      padding: const EdgeInsets.all(20),
      decoration: BoxDecoration(color: isDark ? const Color(0xFF1E1E1E) : Colors.white, borderRadius: BorderRadius.circular(24), boxShadow: [BoxShadow(color: Colors.black.withOpacity(0.05), blurRadius: 10)]),
      child: Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text(title, style: TextStyle(color: isDark ? Colors.white : Colors.black, fontWeight: FontWeight.bold, fontSize: 16, fontFamily: 'Google Sans Flex')), const SizedBox(height: 4), Text(subtitle, style: TextStyle(color: isDark ? Colors.white70 : Colors.black54, fontSize: 13, fontFamily: 'Google Sans Flex'))]), Icon(Icons.arrow_forward_ios_rounded, color: badgeColor, size: 18)]),
    );
  }

  Widget _buildChatTabContent(Size size, double padding, bool isDark) {
    final textColor = isDark ? Colors.white : const Color(0xFF1E1E1E);
    final typeFilter = _selectedChatTab == 0 ? 'classmate' : _selectedChatTab == 1 ? 'teacher' : 'group';
    final filteredChats = _chats.where((c) => c['chat_type'] == typeFilter).toList();
    return Stack(
  ...
  Widget _buildLiquidGlassChatTabs(bool isDark) {
    return ClipRRect(borderRadius: BorderRadius.circular(30), child: BackdropFilter(filter: ImageFilter.blur(sigmaX: 20, sigmaY: 20), child: Container(padding: const EdgeInsets.all(6), decoration: BoxDecoration(color: isDark ? Colors.white.withOpacity(0.08) : Colors.white.withOpacity(0.6), borderRadius: BorderRadius.circular(30), border: Border.all(color: isDark ? Colors.white.withOpacity(0.15) : Colors.white.withOpacity(0.8), width: 1.2)), child: Row(children: [_buildChatTabItem(0, 'Classmates', isDark), _buildChatTabItem(1, 'Teachers', isDark), _buildChatTabItem(2, 'Groups', isDark)]))));
  }

  Widget _buildChatTabItem(int index, String label, bool isDark) {
    final isSelected = _selectedChatTab == index;
    return Expanded(child: GestureDetector(onTap: () { if (hapticsNotifier.value) HapticFeedback.selectionClick(); setState(() => _selectedChatTab = index); }, child: AnimatedContainer(duration: smoothMotionNotifier.value ? const Duration(milliseconds: 160) : Duration.zero, padding: const EdgeInsets.symmetric(vertical: 10), decoration: BoxDecoration(color: isSelected ? (isDark ? Colors.white.withOpacity(0.2) : Colors.white) : Colors.transparent, borderRadius: BorderRadius.circular(24), boxShadow: isSelected ? [BoxShadow(color: Colors.black.withOpacity(0.05), blurRadius: 5)] : []), child: Center(child: Text(label, style: TextStyle(color: isSelected ? (isDark ? Colors.white : Colors.black) : (isDark ? Colors.white54 : Colors.black54), fontWeight: isSelected ? FontWeight.bold : FontWeight.w500, fontSize: 13, fontFamily: 'Google Sans Flex'))))));
  }

  Widget _buildChatTile(String chatId, String title, String chatType, bool isDark) {
    return GestureDetector(onTap: () => Navigator.push(context, PageRouteBuilder(pageBuilder: (context, animation, secondaryAnimation) => ChatScreen(chatId: chatId, chatName: title, chatType: chatType), transitionsBuilder: (context, animation, secondaryAnimation, child) => FadeTransition(opacity: animation, child: child))), child: Container(margin: const EdgeInsets.only(bottom: 12), padding: const EdgeInsets.all(16), decoration: BoxDecoration(color: isDark ? const Color(0xFF1E1E1E) : Colors.white, borderRadius: BorderRadius.circular(20), boxShadow: [BoxShadow(color: Colors.black.withOpacity(0.03), blurRadius: 10)]), child: Row(children: [CircleAvatar(backgroundColor: _accentBlue.withOpacity(0.2), child: Icon(chatType == 'classmate' ? Icons.person : chatType == 'teacher' ? Icons.school : Icons.group, color: _accentBlue)), const SizedBox(width: 16), Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text(title, style: TextStyle(color: isDark ? Colors.white : Colors.black, fontWeight: FontWeight.bold, fontSize: 16, fontFamily: 'Google Sans Flex')), const SizedBox(height: 4), Text('Tap to chat... (Long press to delete)', style: TextStyle(color: isDark ? Colors.white54 : Colors.black54, fontSize: 12, fontFamily: 'Google Sans Flex'))])), Icon(Icons.arrow_forward_ios_rounded, color: isDark ? Colors.white30 : Colors.black26, size: 16)])));
  }
  ...
  Widget _buildSidebarItem({required IconData icon, required String title, required int index, required bool isDark}) {
    ...
    return Container(decoration: BoxDecoration(color: isSelected ? (isDark ? Colors.white.withOpacity(0.15) : Colors.white.withOpacity(0.7)) : Colors.transparent, borderRadius: BorderRadius.circular(20), border: isSelected ? Border.all(color: isDark ? Colors.white24 : Colors.white, width: 1.5) : null), child: ListTile(leading: Icon(icon, color: isSelected ? _accentBlue : (isDark ? Colors.white60 : Colors.black54)), title: Text(title, style: TextStyle(color: isSelected ? (isDark ? Colors.white : Colors.black) : (isDark ? Colors.white70 : Colors.black87), fontWeight: isSelected ? FontWeight.bold : FontWeight.w500, fontFamily: 'Google Sans Flex')), onTap: () { setState(() => _currentIndex = index); Navigator.pop(context); }));
  }

  Widget _buildLiquidGlassNavBar(Size size, bool isDark) {
    return Column(mainAxisSize: MainAxisSize.min, children: [GestureDetector(onTap: () => Navigator.push(context, MaterialPageRoute(builder: (_) => const AnnouncementsScreen())), child: ClipRRect(borderRadius: BorderRadius.circular(30), child: BackdropFilter(filter: ImageFilter.blur(sigmaX: 15, sigmaY: 15), child: Container(padding: const EdgeInsets.symmetric(horizontal: 20, vertical: 10), decoration: BoxDecoration(color: isDark ? Colors.white.withOpacity(0.12) : Colors.white.withOpacity(0.65), borderRadius: BorderRadius.circular(30), border: Border.all(color: isDark ? Colors.white.withOpacity(0.2) : Colors.white.withOpacity(0.8), width: 1.2)), child: Row(mainAxisSize: MainAxisSize.min, children: [Icon(Icons.campaign_outlined, color: isDark ? Colors.white : const Color(0xFF1E1E1E), size: 22), const SizedBox(width: 8), Text('Announcements', style: TextStyle(color: isDark ? Colors.white : const Color(0xFF1E1E1E), fontSize: 15, fontWeight: FontWeight.w600, fontFamily: 'Google Sans Flex'))]))))), const SizedBox(height: 8), ClipRRect(borderRadius: BorderRadius.circular(36), child: BackdropFilter(filter: ImageFilter.blur(sigmaX: 20, sigmaY: 20), child: Container(padding: const EdgeInsets.all(8), decoration: BoxDecoration(color: isDark ? Colors.black.withOpacity(0.45) : const Color(0xFFD6E2EA).withOpacity(0.6), borderRadius: BorderRadius.circular(36), border: Border.all(color: isDark ? Colors.white.withOpacity(0.15) : Colors.white.withOpacity(0.7), width: 1.5)), child: Row(mainAxisAlignment: MainAxisAlignment.spaceAround, children: [_buildGlassNavItem(0, Icons.home_rounded, 'Home', isDark), _buildGlassNavItem(1, Icons.assignment_rounded, 'Classwork', isDark), _buildGlassNavItem(2, Icons.auto_awesome_rounded, 'Notebook', isDark), _buildGlassNavItem(3, Icons.chat_bubble_outline_rounded, 'Chat', isDark)]))))]);
  }

  Widget _buildGlassNavItem(int index, IconData icon, String label, bool isDark) {
    final isSelected = _currentIndex == index;
    return Expanded(child: GestureDetector(onTap: () { if (hapticsNotifier.value) HapticFeedback.lightImpact(); setState(() => _currentIndex = index); }, child: AnimatedContainer(duration: smoothMotionNotifier.value ? const Duration(milliseconds: 160) : Duration.zero, padding: const EdgeInsets.symmetric(vertical: 10), decoration: BoxDecoration(color: isSelected ? (isDark ? Colors.white.withOpacity(0.2) : Colors.white.withOpacity(0.85)) : Colors.transparent, borderRadius: BorderRadius.circular(28)), child: Column(mainAxisSize: MainAxisSize.min, children: [Icon(icon, color: isSelected ? (isDark ? Colors.white : Colors.black) : (isDark ? Colors.white54 : const Color(0xFF445566)), size: 24), const SizedBox(height: 3), Text(label, style: TextStyle(color: isSelected ? (isDark ? Colors.white : Colors.black) : (isDark ? Colors.white54 : const Color(0xFF445566)), fontSize: 12, fontWeight: isSelected ? FontWeight.bold : FontWeight.w500, fontFamily: 'Google Sans Flex'))]))));
  }
  ...
  void _showEditNameDialog(bool isDark) {
    ...
    showDialog(context: context, barrierColor: Colors.black.withOpacity(0.4), builder: (context) => _GlassDialogBase(isDark: isDark, accentColor: _accentBlue, title: 'Change Display Name', icon: Icons.badge_rounded, actionText: 'Save', content: _buildGlassTextField(controller: nameController, hintText: 'New Display Name', icon: Icons.person_outline_rounded, isDark: isDark), onAction: () async { if (hapticsNotifier.value) HapticFeedback.mediumImpact(); Navigator.pop(context); await _updateUserName(nameController.text); }));
  }

  Widget _buildGlassTextField({required TextEditingController controller, required String hintText, required IconData icon, required bool isDark, int maxLines = 1}) {
    return Container(decoration: BoxDecoration(color: isDark ? Colors.black.withOpacity(0.3) : Colors.white.withOpacity(0.5), borderRadius: BorderRadius.circular(16), border: Border.all(color: isDark ? Colors.white12 : Colors.white70)), child: TextField(controller: controller, maxLines: maxLines, style: TextStyle(color: isDark ? Colors.white : Colors.black, fontFamily: 'Google Sans Flex'), decoration: InputDecoration(hintText: hintText, hintStyle: TextStyle(color: isDark ? Colors.white54 : Colors.black54, fontFamily: 'Google Sans Flex'), prefixIcon: Icon(icon, color: isDark ? Colors.white70 : Colors.black54), border: InputBorder.none, contentPadding: const EdgeInsets.symmetric(horizontal: 16, vertical: 14))));
  }
}
