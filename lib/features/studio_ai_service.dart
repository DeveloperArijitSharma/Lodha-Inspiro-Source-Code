...
class StudioAiService {
  ...
  Future<String?> _call(String endpoint, String key, String model, String prompt) async {
    ...
    }, body: jsonEncode({'model': model, 'messages': [{'role': 'system', 'content': 'You create safe, clear, age-appropriate student classwork. Return only the format requested.'}, {'role': 'user', 'content': prompt}], 'temperature': 0.35, 'max_tokens': 2500}));
    if (r.statusCode >= 200 && r.statusCode < 300) {
      final d = jsonDecode(r.body) as Map<String, dynamic>;
      final choices = d['choices'] as List<dynamic>?;
      final content = choices?.isNotEmpty == true ? (choices!.first['message'] as Map<String, dynamic>?)?['content'] : null;
