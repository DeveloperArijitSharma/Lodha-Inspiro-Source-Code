...
class FileManagerService {
  ...
  static const Set<String> supportedExtensions = {'pdf','txt','md','docx','xlsx','pptx','jpg','jpeg','png','webp'};
  ...
  String _extension(String name) => name.contains('.') ? name.split('.').last.toLowerCase() : '';
  ...
  String classify(String name, {String? sourceKind}) {
    ...
    if (sourceKind == 'ai' || lower.contains('ai') || lower.contains('prompt') || lower.contains('assistant')) return 'ai_work';
  ...
  Future<String> _extractText(String extension, Uint8List bytes) async {
    ...
    try { return PdfTextExtractor(document).extractText().trim(); } finally { document.dispose(); }
  }
  ...
  Future<void> pickAndUpload() async {
    final result = await FilePicker.platform.pickFiles(allowMultiple: true, withData: true, type: FileType.custom, allowedExtensions: supportedExtensions.toList());
    if (result == null) return;
    final user = currentUserId;
    if (user == null) throw StateError('Please sign in again.');
    for (final picked in result.files) {
      final bytes = picked.bytes;
      if (bytes != null && bytes.isNotEmpty) await uploadBytes(user, picked.name, bytes);
    }
  }

  Future<void> uploadBytes(String userId, String name, Uint8List bytes, {String? sourceKind, String visibility = 'personal'}) async {
    final extension = _extension(name);
    if (!supportedExtensions.contains(extension)) throw ArgumentError('This file type is not supported.');
    final id = _uuid.v4();
    final safeName = name.replaceAll(RegExp(r'[^A-Za-z0-9._ -]'), '_');
    final storagePath = visibility == 'school' ? 'school/${id}_$safeName' : '$userId/${id}_$safeName';
    final contentText = await _extractText(extension, bytes);
    final category = classify(name, sourceKind: sourceKind);
    await _supabase.storage.from(bucket).uploadBinary(storagePath, bytes, fileOptions: FileOptions(contentType: _mime(extension), upsert: false));
    try {
      await _supabase.from('file_manager_files').insert({'owner_id': userId, 'name': name, 'storage_path': storagePath, 'extension': extension, 'mime_type': _mime(extension), 'size_bytes': bytes.length, 'category': category, 'visibility': visibility, 'ai_supported': aiSupportedExtensions.contains(extension), 'content_text': contentText.isEmpty ? null : contentText, 'metadata': {'source_kind': sourceKind}});
    } catch (_) {
  ...
  Future<void> rename(Map<String, dynamic> file, String newName) async {
    ...
    try { await _supabase.from('file_manager_files').update({'name': trimmed, 'storage_path': newPath}).eq('id', file['id']); }
    catch (_) { await _supabase.storage.from(bucket).move(newPath, oldPath); rethrow; }
  }
  ...
  Future<void> updateTextFile(Map<String, dynamic> file, String text) async {
    ...
    await _supabase.from('file_manager_files').update({'content_text': text, 'size_bytes': bytes.length}).eq('id', file['id']);
  }
  ...
  Future<void> analyzeAndOrganize() async {
    ...
    for (final file in files) {
      ...
      await _supabase.from('file_manager_files').update({'category': category, 'ai_supported': aiSupportedExtensions.contains(extension)}).eq('id', file['id']);
    }
  }
}
