      ...
      builder: (sheetContext) => CupertinoActionSheet(
        ...
        actions: [
          CupertinoActionSheetAction(
            ...
            child: const Text('Files / Drive / OneDrive / Other apps'),
          ),
          CupertinoActionSheetAction(
          ...
          CupertinoActionSheetAction(
            onPressed: () => Navigator.pop(sheetContext, 'folder'),
  ...
  Future<void> _openFile(Map<String, dynamic> file) async {
    ...
    if (ext == 'txt' || ext == 'md') {
      final changed = await Navigator.push<bool>(
        context,
        MaterialPageRoute(builder: (_) => FileEditorScreen(file: file)),
      );
    ...
    try {
      ...
      await Navigator.push(
        context,
        MaterialPageRoute(
          builder: (_) => FilePreviewScreen(file: file, bytes: bytes),
        ),
      );
    } catch (_) {
  ...
  Future<void> _moveFile(Map<String, dynamic> file) async {
    ...
    final target = await showCupertinoModalPopup<String?>(
      ...
      builder: (sheetContext) => CupertinoActionSheet(
        ...
        actions: [
          CupertinoActionSheetAction(
            onPressed: () => Navigator.pop(sheetContext, '__root__'),
            child: const Text('Personal root'),
          ),
          ...folders.map(
            (folder) => CupertinoActionSheetAction(
              onPressed: () => Navigator.pop(sheetContext, folder['id'].toString()),
    ...
    await _service.moveFile(
      file,
      folderId: target == '__root__' ? null : target,
    );
    await _refresh();
  }
  ...
  Future<void> _sortSheet() async {
    final selected = await showCupertinoModalPopup<_SortMode>(
      ...
      builder: (sheetContext) => CupertinoActionSheet(
        ...
        actions: [
          CupertinoActionSheetAction(
            onPressed: () => Navigator.pop(sheetContext, _SortMode.name),
            child: const Text('Name'),
          ),
          CupertinoActionSheetAction(
            onPressed: () => Navigator.pop(sheetContext, _SortMode.type),
            child: const Text('Type'),
          ),
          CupertinoActionSheetAction(
            onPressed: () => Navigator.pop(sheetContext, _SortMode.size),
            child: const Text('Size'),
          ),
          CupertinoActionSheetAction(
            onPressed: () => Navigator.pop(sheetContext, _SortMode.newest),
            child: const Text('Date & time, newest first'),
          ),
          CupertinoActionSheetAction(
            onPressed: () => Navigator.pop(sheetContext, _SortMode.oldest),
            child: const Text('Date & time, oldest first'),
          ),
        ],
        cancelButton: CupertinoActionSheetAction(
          onPressed: () => Navigator.pop(sheetContext),
                ...
                children: [
                  ...
                  Row(
                    children: [
                      Expanded(
                        child: Text(
                          '${_folders.length} folders • ${_visibleFiles.length} files',
                          style: TextStyle(
                  ...
                  if (_loading)
                    ...
                    if (_folders.isNotEmpty) ...[
                      _SectionTitle('Folders', foreground),
                      const SizedBox(height: 8),
                      ..._folders.map(
                        (folder) => _FolderTile(
                          folder: folder,
                    ...
                    if (_visibleFiles.isNotEmpty) ...[
                      _SectionTitle('Files', foreground),
                      const SizedBox(height: 8),
                      ..._visibleFiles.map(
                        (file) => _FileTile(
                          file: file,
                    ...
                    if (_folders.isEmpty && _visibleFiles.isEmpty)
                      _EmptyFiles(
                        dark: dark,
                        onImport: _import,
                        onFolder: _createFolder,
    ...
    return Stack(
      children: [
        Positioned(
          top: -120,
          right: -100,
          child: _orb(300, const Color(0xFF4B8DFF)),
        ),
        Positioned(
          top: 240,
          left: -150,
          child: _orb(320, const Color(0xFF9A7BFF)),
        ),
        Positioned(
          bottom: -180,
          right: -120,
          child: _orb(360, const Color(0xFF56D8C0)),
        ),
        Positioned.fill(
...
class _FolderHeader extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Row(
      children: [
        if (!isRoot)
          _GlassButton(icon: CupertinoIcons.chevron_left, onTap: onBack)
        else
          ...
          child: Column(
            ...
            children: [
              Text(
                'File Manager',
                style: TextStyle(
                  color: foreground.withOpacity(.5),
                  fontSize: 12,
                ),
              ),
              Text(
        ...
        _GlassButton(icon: CupertinoIcons.folder_badge_plus, onTap: onFolder),
        const SizedBox(width: 6),
        _GlassButton(icon: CupertinoIcons.cloud_download, onTap: onImport),
        const SizedBox(width: 6),
        _GlassButton(icon: CupertinoIcons.sparkles, onTap: onOrganize),
      ],
    );
  }
}
  ...
  Widget build(BuildContext context) {
    ...
    return ClipRRect(
      ...
      child: BackdropFilter(
        ...
        child: Material(
          color: dark ? Colors.white.withOpacity(.08) : Colors.white.withOpacity(.72),
          child: InkWell(
            onTap: onTap,
            child: Padding(
              ...
              child: Icon(
                icon,
                size: 19,
                color: dark ? Colors.white : const Color(0xFF1A2A43),
              ),
            ),
          ),
        ),
                    ...
                    Expanded(
                      child: Column(
                        ...
                        children: [
                          ...
                          Text(
                            '${ext.toUpperCase()} • ${_size(file['size_bytes'])}${ai ? ' • AI ready' : ''}',
                            style: TextStyle(
                              color: foreground.withOpacity(.5),
                              fontSize: 11.5,
                            ),
...
class _EmptyFiles extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      decoration: BoxDecoration(
        color: dark ? Colors.white.withOpacity(.06) : Colors.white.withOpacity(.72),
      ...
      child: Column(
        children: [
          Icon(
            ...
            size: 54,
            color: dark ? Colors.white38 : const Color(0xFF9AA9BD),
          ),
          ...
          Text(
            'Create folders or import from Files, Gallery, Drive, OneDrive and other apps.',
...
class FilePreviewScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final dark = Theme.of(context).brightness == Brightness.dark;
    return Scaffold(
      backgroundColor: dark ? const Color(0xFF090D14) : const Color(0xFFF4F7FB),
      appBar: AppBar(
        title: Text(file['name'].toString()),
        leading: IconButton(
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 8),
            const Text(
              'The original file is stored unchanged. This format can be opened or shared from the file actions.',
              textAlign: TextAlign.center,
            ),
          ],
        ),
